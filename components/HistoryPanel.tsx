...
import { getHistory, deleteHistoryItem, clearHistory, exportDatabase, importDatabase } from '../services/historyService';
import { parseGeneratedArticle } from '../services/promptService';
import { Clock, Trash2, ArrowUpRight, AlignLeft, Image as ImageIcon, AlertCircle, Loader2, Download, Upload, Database } from 'lucide-react';
...
const HistoryPanel: React.FC<Props> = ({ onRestore }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!confirm("Importing will merge backup data into your current history. Continue?")) {
      ...
      reader.onload = async (event) => {
          try {
              ...
              const count = await importDatabase(json);
              alert(`Successfully restored ${count} items.`);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
         ...
         <div className="flex items-center gap-2">
            ...
            <button 
                ...
                className="text-xs font-medium text-slate-600 hover:text-indigo-600 bg-white border border-slate-200 hover:border-indigo-300 px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 shadow-sm"
            ...
            <button 
                onClick={handleClear}
                className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1 px-3 py-2 rounded hover:bg-red-50 transition-colors"
      ...
      {history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-slate-400 bg-white border border-slate-200 border-dashed rounded-xl">
            <Database className="w-12 h-12 mb-4 opacity-20" />
            <h3 className="text-lg font-medium text-slate-600">No History Yet</h3>
            <p className="text-sm mb-6">Generated articles will appear here.</p>
            <div className="flex gap-3">
                <button onClick={handleImportClick} className="text-xs text-indigo-600 font-medium hover:underline">
                    Import Backup
                </button>
            </div>
        </div>
      ...
      <div className="text-center text-xs text-slate-400 flex items-center justify-center gap-1 mt-8">
        ...
        <span>Pro Tip: Regularly export backups to keep your data safe.</span>
      </div>
    </div>
  );
};
